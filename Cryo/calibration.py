...
class CalibrationPanel(QWidget):
    ...
    def __init__(self, setup_getter, config_getter=None, parent=None):
        ...
        self.fl_combo = QLineEdit("(set in Setup Defaults tab)")
        self.fl_combo.setReadOnly(True)
        self.fl_combo.setStyleSheet(
            "background:#1e1e2e;color:#6c7086;border:1px solid #313244;"
            "border-radius:4px;padding:2px 4px;")
        self.fl_combo.setToolTip("Set in the Setup Defaults tab")
        ...
        self.focus_pos_spin.setToolTip(
            "Scan-axis coordinate (X or Y) where the sample is typically centred.\n"
            "The autofocus routine moves the scan axis here before sweeping Z.")
        ...
        self.dz_spin.setToolTip(
            "Initial Z step size per autofocus iteration.\n"
            "Smaller = finer search but slower. Larger = faster but may overshoot.\n"
            "The step is automatically reduced as the focus is approached.")
        ...
        self.dzmax_spin.setToolTip(
            "Maximum total Z travel allowed from the starting position.\n"
            "If the focus is not found within this range, the stage returns to Z₀.")
        ...
        self.tries_spin.setToolTip(
            "Maximum number of Z steps before the autofocus routine gives up.\n"
            "Each step includes a settle delay (~0.75 s) plus a readout (~0.3 s).")
    ...
    def set_fl_device(self, dev: str):
        """Set the FL sensor device display (called from Setup Defaults)."""
        self.fl_combo.setText(dev or "(set in Setup Defaults tab)")
    ...
    def _start_autofocus(self):
        ...
        x_dev  = cfg.get("act1_device", s.get("act1_device", ""))
        z_dev  = s.get("z_device", x_dev)
        z_attr = s.get("z_attr", cfg.get("z_attr", "position0"))
